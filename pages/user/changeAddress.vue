		...
		methods: {
			...
			saveAddress() {
				...
				if (!data.name) {
					...
					let list6=self.addressList;
					let list3=[];
					let list4=list6.splice(self.index,0,list1);
					let postData={
						"receiveAddressJson":JSON.stringify(list6)
					}
					self.Post2('/api/user/updateUserInfo',postData ).then(res => {
						if (res.code == 200) {
							uni.showToast({
								title: '地址保存成功'
							});
